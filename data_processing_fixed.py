# modules/data_processing.py (Versión 3.3 - Corregida)
"""
Versión 3.3 Correcciones Críticas:
- Criterio de revisión MÁS ESTRICTO (pide revisión si falta nombre, RUT, monto o convenio)
- Mejor extracción de horas (más patrones)
- Búsqueda en memoria del lote actual (registros ya procesados)
- Periodo de servicio solo con mes (YYYY-MM)
"""
import re
import difflib
...
class FieldExtractor:
    ...
    def extract_horas(self, text: str, glosa: str = "") -> str:
        ...
        patrones = [
            # "44 horas" o "44 hrs" o "44 h"
            r'(\d{1,3})\s*(?:hrs?|horas?)\b',
            # "Horas: 44" o "Hrs: 44"
            r'\b(?:hrs?|horas?)[\s:]+(\d{1,3})\b',
            # "44 H" (mayúscula)
            r'(\d{1,3})\s*H\b',
            # Líneas que contienen "horas trabajadas" seguido de número
...
class DataProcessorOptimized:
    """Procesador Ultra-Mejorado v3.3 - CORREGIDO"""
    ...
    def __init__(self):
        ...
        self.memory = Memory()
        # NUEVO: Caché de registros procesados en esta sesión
        self.session_cache = []
    ...
    def process_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            # Paso 5: Buscar en registros ya procesados de esta sesión
            campos = self._buscar_en_lote_actual(campos)
            
            # Paso 6: Validar monto/horas
            ...
            # Paso 8: Decisión de revisión (MÁS ESTRICTA)
    ...
    def _buscar_en_lote_actual(self, campos: Dict) -> Dict:
        """NUEVO: Busca en registros ya procesados de esta sesión"""
        rut = campos.get('rut', '').strip()
        nombre = campos.get('nombre', '').strip()
        
        # Si falta RUT pero tengo nombre, buscar en sesión actual
        if nombre and not rut:
        ...
        # Si falta nombre pero tengo RUT, buscar en sesión actual
        if rut and (not nombre or campos.get('nombre_confidence', 0) < 0.5):
            for registro_previo in self.session_cache:
                rut_previo = registro_previo.get('rut', '').strip()
                nombre_previo = registro_previo.get('nombre', '').strip()
        ...
        # Si falta convenio pero tengo RUT, buscar en sesión actual
        convenio_actual = campos.get('convenio', '').strip()
        if (not convenio_actual or campos.get('convenio_confidence', 0) < 0.4) and rut:
            for registro_previo in self.session_cache:
                rut_previo = registro_previo.get('rut', '').strip()
    ...
    def _segunda_pasada_desde_glosa(self, campos: Dict, texto_completo: str) -> Dict:
        ...
        # Reintentar HORAS si falta (MEJORADO)
    ...
    def _needs_review_strict(self, campos: Dict, confianza: float) -> bool:
        """
        ¡VERSIÓN CORREGIDA!
        Criterio MÁS ESTRICTO: Pedir revisión si falta CUALQUIERA de los campos críticos:
        - nombre
        - RUT
        - monto
        - convenio
        
        O si la confianza es muy baja
        """
        ...
        # Pedir revisión si falta CUALQUIER campo crítico
        if not tiene_rut:
            return True
        ...
        # También revisar si confianza es muy baja
        if confianza < 0.35:
            return True
