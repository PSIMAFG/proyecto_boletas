# modules/data_processing.py (v3.3 FINAL - Criterios Balanceados)
"""
Versión 3.3 FINAL - Corrección definitiva:
- Extracción mejorada con doble pasada
- Búsqueda bidireccional en memoria
- Criterios de revisión BALANCEADOS (no muy estrictos, no muy relajados)
- SIEMPRE revisar si falta convenio (crítico para resumen financiero)
- SIEMPRE revisar si faltan múltiples campos críticos
"""
import re
import difflib
...
class FieldExtractor:
    ...
    def extract_from_glosa(self, glosa: str, campo: str) -> Tuple[str, float]:
        ...
        if campo == 'fecha':
            ...
            # Intentar extraer nombre desde glosa si menciona "Señor(es):" o similar
...
class DataProcessorOptimized:
    """Procesador v3.3 FINAL con criterios balanceados"""
    ...
    def process_file(self, file_path: Path) -> Dict:
        """Procesa archivo con extracción mejorada y criterios balanceados"""
        try:
            # Paso 1: OCR
            ...
            # Paso 3: SEGUNDA PASADA - Reintentar campos faltantes desde glosa
            campos = self._segunda_pasada_desde_glosa(campos, texto_completo)
            
            # Paso 4: Autocompletar con memoria (después de extraer todo lo posible)
            ...
            # Paso 8: Decisión de revisión BALANCEADA
            campos['needs_review'] = self._needs_review_balanced(campos, confianza_promedio)
            campos['quality_score'] = self._calculate_quality(campos)
            
            # Log de diagnóstico para casos específicos
            nombre_archivo = file_path.stem.lower()
            if any(n in nombre_archivo for n in ['valezka', 'sarella', 'daniel', 'elizabeth', 'alexandros']):
                self._log_diagnostico(campos, file_path)
    ...
    def _autofill_inteligente(self, campos: Dict) -> Dict:
        ...
        # CASO 3: Autocompletar convenio SI NO HAY NADA
        convenio_actual = campos.get('convenio', '').strip()
        if not convenio_actual and rut:
    ...
    def _needs_review_balanced(self, campos: Dict, confianza: float) -> bool:
        """
        Criterio BALANCEADO para revisión manual.
        NO muy estricto, NO muy relajado.
        
        CAMPOS CRÍTICOS PARA FINANZAS:
        1. RUT (identificación)
        2. Nombre (identificación)
        3. Monto (finanzas)
        4. Fecha (periodo)
        5. Convenio (CRÍTICO para resumen financiero)
        """
        ...
        # CRITERIO 1: Si falta convenio → SIEMPRE REVISAR
        # El convenio es CRÍTICO para el resumen financiero
        if not tiene_convenio:
            campos['revision_reason'] = 'Falta convenio (crítico para resumen financiero)'
            return True
        
        # CRITERIO 2: Si faltan 2 o más campos críticos → REVISAR
        campos_faltantes = 0
        faltantes_nombres = []
        
        if not tiene_rut:
            campos_faltantes += 1
            faltantes_nombres.append('RUT')
        if not tiene_nombre:
            campos_faltantes += 1
        ...
        if campos_faltantes >= 2:
            campos['revision_reason'] = f'Faltan campos críticos: {", ".join(faltantes_nombres)}'
            return True
        
        # CRITERIO 3: Si falta monto (crítico) → REVISAR
        if not tiene_monto:
            campos['revision_reason'] = 'Falta monto'
            return True
        
        # CRITERIO 4: Si confianza es MUY baja → REVISAR
        if confianza < 0.30:
            campos['revision_reason'] = f'Confianza muy baja: {confianza:.1%}'
            return True
        
        # CRITERIO 5: Validaciones adicionales
        
        # Si tiene RUT pero es inválido → REVISAR
        if tiene_rut:
            from modules.utils import dv_ok
            if not dv_ok(campos['rut']):
                campos['revision_reason'] = 'RUT con dígito verificador inválido'
                return True
        
        # Si el monto está fuera de rango esperado → REVISAR
        if tiene_monto:
            try:
        ...
        # Si la fecha es sospechosa → REVISAR
        if tiene_fecha:
            try:
                from datetime import datetime
        ...
        # Si llegamos aquí, no necesita revisión
        return False
    
    def _log_diagnostico(self, campos: Dict, file_path: Path):
        """Log de diagnóstico para casos específicos"""
        print(f"\n=== DIAGNÓSTICO: {file_path.stem} ===")
        print(f"RUT: {campos.get('rut', 'FALTA')} (conf: {campos.get('rut_confidence', 0):.2f})")
        print(f"Nombre: {campos.get('nombre', 'FALTA')} (conf: {campos.get('nombre_confidence', 0):.2f})")
        print(f"Fecha: {campos.get('fecha_documento', 'FALTA')} (conf: {campos.get('fecha_confidence', 0):.2f})")
        print(f"Monto: {campos.get('monto', 'FALTA')} (conf: {campos.get('monto_confidence', 0):.2f})")
        print(f"Convenio: {campos.get('convenio', 'FALTA')} (conf: {campos.get('convenio_confidence', 0):.2f})")
        print(f"Needs Review: {campos.get('needs_review')}")
        if campos.get('revision_reason'):
            print(f"Razón: {campos['revision_reason']}")
        print("=" * 40)
    ...
    def _calculate_quality(self, campos: Dict) -> float:
        ...
        pesos = {
            ...
            'monto': 0.25,
            'nombre': 0.15,
            'convenio': 0.10,  # Convenio es importante
            'glosa': 0.05
        }
