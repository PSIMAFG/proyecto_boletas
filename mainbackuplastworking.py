# main.py (VersiÃ³n Mejorada 3.1)
"""
AplicaciÃ³n principal mejorada con extracciÃ³n mÃ¡s robusta
"""
...
class ImprovedReviewDialog(tk.Toplevel):
    """DiÃ¡logo mejorado de revisiÃ³n manual"""
    
    def __init__(self, master, row: dict):
        super().__init__(master)
        self.title(f"RevisiÃ³n: {Path(row.get('archivo', '')).name}")
    ...
    def _create_preview(self, parent):
        ...
        ttk.Button(btn_frame, text="ðŸ“‚ Abrir original", 
    ...
    def _create_fields(self, parent):
        ...
        if self.row.get('valor_hora_calculado'):
            ...
            if self.row.get('monto_fuera_rango'):
                info += "\nâš ï¸ Monto parece fuera de rango esperado"
        ...
        ttk.Button(btn_frame, text="âœ“ Guardar", 
                  command=self._on_save).pack(side="left", padx=2)
        ttk.Button(btn_frame, text="âŠ˜ Omitir", 
...
class BoletasApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Label(row1, text="ðŸ“ Carpeta entrada:").pack(side="left")
        ...
        ttk.Label(row2, text="ðŸ“Š Archivo salida:").pack(side="left")
        ...
        ttk.Checkbutton(options_frame, text="âœ“ RevisiÃ³n manual de registros dudosos",
                       variable=self.var_manual_review).pack(anchor="w")
        ttk.Checkbutton(options_frame, text="ðŸ“Š Generar informes por convenio",
        ...
        self.btn_start = ttk.Button(control_frame, text="â–¶ Iniciar Procesamiento",
                                    command=self.start_processing)
        self.btn_start.pack(side="left", padx=5)
        
        self.btn_stop = ttk.Button(control_frame, text="â¸ Detener",
                                   command=self.stop_processing,
                                   state="disabled")
        self.btn_stop.pack(side="left", padx=5)
        
        ttk.Button(control_frame, text="âŒ‚ Salir", command=self.quit).pack(side="right", padx=5)
    ...
    def check_dependencies(self):
        ...
        if detect_tesseract_cmd():
            self.log("Tesseract: OK âœ“", "success")
        else:
            self.log("Tesseract: NO ENCONTRADO", "warning")
        
        if detect_poppler_bin():
            self.log("Poppler: OK âœ“", "success")
        else:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result.get('error'):
                            ...
                            self.log(f"âœ— Error: {file_path.name} - {result.get('error')}", "error")
                        elif result.get('needs_review'):
                            review_queue.append(result)
                            self.log(f"âš  Revisar: {file_path.name}", "warning")
                        else:
                            results.append(result)
                            quality = result.get('quality_score', 0)
                            self.log(f"âœ“ OK: {file_path.name} (Q:{quality:.0%})", "success")
                    
                    except Exception as e:
                        errors.append(str(file_path))
                        self.log(f"âœ— Error: {file_path.name} - {e}", "error")
                    
                    self.update_idletasks()
            
            # RevisiÃ³n manual
            if self.var_manual_review.get() and review_queue:
                self.log(f"Iniciando revisiÃ³n manual: {len(review_queue)} boletas", "info")
    ...
    def _manual_review_process(self, review_queue):
        ...
        for i, record in enumerate(review_queue):
            ...
            if dialog.result:
                ...
                self.log(f"âœ“ Revisado: {Path(record['archivo']).name}", "success")
            else:
                self.log(f"âŠ˜ Omitido: {Path(record['archivo']).name}", "warning")
    ...
    def _generate_excel(self, results):
        ...
        try:
            ...
            # 2) Intentar reemplazo atÃ³mico
            try:
                os.replace(tmp_file, output_file)  # sobrescribe si existe
                self.log(f"âœ“ Excel generado: {output_file}", "success")
            except PermissionError:
                # 3) Si estÃ¡ bloqueado, guardamos con nombre alternativo
                ...
                self.log("âš  Archivo destino en uso (Excel abierto). "
                        f"Guardado como: {alt}", "warning")

            self.log(f"  Total registros: {len(df)}", "info")

            # Solo preguntar abrir si se generÃ³ con Ã©xito sin lock
            if output_file.exists():
                if messagebox.askyesno("Completado", "Â¿Abrir el archivo Excel?"):
                    try:
            ...
            # Limpieza del temporal si quedÃ³ tirado
            try:
...
def main():
    ...
    app.log("VersiÃ³n mejorada con extracciÃ³n robusta", "info")
    app.log("=" * 50, "info")
    app.log("Listo para procesar âœ“", "success")
