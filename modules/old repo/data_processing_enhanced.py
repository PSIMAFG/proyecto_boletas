...
from modules.ocr_extraction_enhanced import EnhancedOCRExtractor

class EnhancedDataProcessor:
    """Procesador de datos mejorado con soporte multi-motor"""
    
    def __init__(self, engine: OCREngine = OCREngine.AUTO):
        self.ocr_extractor = EnhancedOCRExtractor(engine)
    ...
    def extract_number_tokens_from_line(self, ln: str) -> List[str]:
        ...
        for m in MONEDA_SIM_RE.finditer(ln):
            raw = m.group(1)
            raw = re.match(r'^[0-9][0-9\.\,\s]*', raw).group(0) if re.match(r'^[0-9][0-9\.\,\s]*', raw) else ""
            if raw:
    ...
    def process_file(self, path: Path) -> Dict:
        """Procesa un archivo completo con el motor OCR configurado"""
        ext = path.suffix.lower()
        
        if ext == ".pdf":
            # Primero verificar texto embebido
            embedded_text = self.ocr_extractor.extract_text_from_pdf_embedded(path)
            
            if embedded_text and self.ocr_extractor.check_if_text_is_readable(embedded_text):
                # Usar texto embebido
                fields = self.extract_fields_from_text(embedded_text, path)
                
                # Si tiene campos clave, usarlo
                if any(fields.get(k) for k in ("rut", "nro_boleta", "monto")):
                    fields['archivo'] = str(path)
                    fields['paginas'] = embedded_text.count("\f") + 1
                    fields['confianza'] = 0.99
                    fields['confianza_max'] = 0.99
                    fields['ocr_engine'] = 'embedded'
                    fields['versions'] = {}
                    fields['needs_review'] = self.needs_manual_review(
                        fields, 0.99, embedded_text, OCR_CONFIDENCE_THRESHOLD
                    )
                    return fields
            
            # Procesar con OCR
            result = self.ocr_extractor.process_pdf_multi_engine(path)
            text_all = "\n".join(result.get('texts', []))
            conf_mean = float(np.mean(result.get('confidences', [0])))
            conf_max = float(np.max(result.get('confidences', [0])))
            
        else:
            # Procesar imagen
            result = self.ocr_extractor.process_image_multi_engine(path)
            text_all = result.get('text', '')
            conf_mean = result.get('confidence', 0.0)
            conf_max = conf_mean
        
        if not text_all:
            raise RuntimeError("No se pudo extraer texto del archivo")
        ...
        # Agregar metadata
        fields['archivo'] = str(path)
        fields['paginas'] = result.get('pages', 1)
        fields['confianza'] = round(conf_mean, 3)
        fields['confianza_max'] = round(conf_max, 3)
        fields['ocr_engine'] = result.get('engine', 'unknown')
        fields['versions'] = result.get('versions', {})
        fields['needs_review'] = self.needs_manual_review(
            fields, conf_mean, text_all, OCR_CONFIDENCE_THRESHOLD
        )
