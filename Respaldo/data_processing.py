...
class FieldExtractor:
    ...
    def extract_fecha(self, text: str) -> Tuple[str, float]:
        ...
        Prioriza:
        1) LÃ­neas con 'Fecha:' (encabezado)
        2) 'Fecha / Hora EmisiÃ³n'
        3) 'Fecha / Hora ImpresiÃ³n'
        Excluye lÃ­neas de ruido: 'Res. Ex', 'www.sii.cl', 'Verifique', 'cÃ³digo verificador', 'D.A.', etc.
        """
        # NormalizaciÃ³n bÃ¡sica para mejorar OCR
        t = re.sub(r'[|]+', ' ', text)
        t = re.sub(r'[ \t]+', ' ', t)
        ...
        ruido = ('res ex', 'res. ex', 'verifique este documento', 'www.sii.cl',
                'codigo verificador', 'cÃ³digo verificador', 'barra', 'resoluciÃ³n',
        ...
        def _is_noise(l: str) -> bool:
            ll = l.lower()
            return any(t in ll for t in ('verifique este documento', 'www.sii.cl', 'res. ex nÂ° 83', 'res. ex n', '2004'))
        ...
        for line in lines:
            l = line.lower()
            # Descarta lÃ­neas de ruido
            if any(tok in l for tok in ruido):
                continue

            score_ctx = -1
            if 'fecha:' in l:
                score_ctx = 3                     # MÃ¡xima prioridad (encabezado)
            elif 'fecha / hora emisiÃ³n' in l or 'fecha/ hora emisiÃ³n' in l or 'fecha / hora emision' in l:
                score_ctx = 2
            elif 'fecha / hora impresiÃ³n' in l or 'fecha/ hora impresiÃ³n' in l or 'fecha / hora impresion' in l:
                score_ctx = 1

            # Intentar ambos formatos en la lÃ­nea
        ...
        # Si no hubo nada con contexto, como Ãºltimo recurso,
        # buscamos globalmente pero manteniendo exclusiones y aÃ±os razonables
        if not candidatos:
            for line in lines:
                l = line.lower()
                if any(tok in l for tok in ruido):
        ...
        # Elegir por mayor score de contexto y, si empata, la fecha mÃ¡s reciente
