...
class ReportGenerator:
    ...
    def _create_main_dataframe(self, registros: List[Dict]) -> pd.DataFrame:
        ...
        # 1) Si viene la columna 'periodo_servicio' desde el extractor, úsala.
        if "periodo_servicio" in df.columns:
            ps_col = df["periodo_servicio"].fillna("").astype(str)
        else:
            ps_col = pd.Series([""] * len(df))

        # 2) Completar 'periodo_servicio' vacío a partir de la glosa (p. ej. "SERVICIO MES DE MARZO 2025")
        #    Si no trae año, lo inferimos con la regla: si mes_servicio > mes(fecha_doc) => año_doc-1, si no => año_doc
        meses_map = {
        ...
        # Solo completa los vacíos
        if ps_col.eq("").any():
            ps_filled = []
            for gl, fdoc, ps in zip(df["glosa"], df["fecha_dt"], ps_col):
                if ps:
                    ps_filled.append(ps)
                else:
                    ps_filled.append(_infer_ps_from_glosa(gl, fdoc))
            ps_col = pd.Series(ps_filled, index=df.index)

        # 3) Convertir a datetime (primer día del mes del servicio)
        def _ps_to_dt(s):
        ...
        # 4) Elegir fuente para mes/año: preferir periodo_dt; si NaT, usar fecha_dt
        df["periodo_final"] = df["periodo_dt"].combine_first(df["fecha_dt"])

        df["mes"] = df["periodo_final"].dt.month
        df["año"] = df["periodo_final"].dt.year
