...
class IntelligentBatchProcessor:
    ...
    def post_process_batch(self, registros: List[Dict], log_callback=None) -> Tuple[List[Dict], List[Dict]]:
        ...
        for registro in registros:
            if self._needs_review_post_process(registro):
    ...
    def _infer_missing_periods(self, registros: List[Dict], log_callback=None) -> List[Dict]:
        """
        NUEVO v4.0: Infiere periodos faltantes desde fechas
        """
        inferidos = 0
        
        for registro in registros:
            if registro.get('mes_nombre') in ['', 'SIN_PERIODO', None]:
                fecha_doc = registro.get('fecha_documento', '')
                
                if fecha_doc:
                    try:
                        fecha_dt = datetime.strptime(fecha_doc, "%Y-%m-%d")
                        
                        # Asumir mes anterior (pago mes vencido)
                        if fecha_dt.month == 1:
                            mes = 12
                            anio = fecha_dt.year - 1
                        else:
                            mes = fecha_dt.month - 1
                            anio = fecha_dt.year
                        
                        registro['mes'] = mes
                        registro['anio'] = anio
                        registro['mes_nombre'] = self.month_names.get(mes, f'Mes {mes}')
                        registro['periodo_servicio'] = f"{anio:04d}-{mes:02d}"
                        inferidos += 1
                    except:
                        pass
        
        if log_callback and inferidos > 0:
            log_callback(f"   ✓ {inferidos} periodos inferidos desde fechas", "success")
...
class DataProcessorOptimized:
    ...
    def _calculate_periodo_basic(self, campos: Dict) -> Dict:
        """Cálculo básico de periodo (refinado en post-proceso)"""
        ...
        # Dejar pendiente para post-procesamiento
        campos['mes'] = None
        campos['anio'] = None
